...
function MobileAdminNav({ pathname, onMore }: { pathname: string; onMore: () => void }) {
  return <nav className="admin-mobile-nav" aria-label="Mobile primary navigation">{navItems.slice(0, 4).map((item) => { const Icon = item.icon; const active = pathname === item.href || pathname.startsWith(item.href + '/'); return <Link href={item.href} key={item.href} className={'admin-mobile-nav-link' + (active ? ' active' : '')} aria-current={active ? 'page' : undefined}><Icon size={18} /><span>{item.label}</span></Link> })}<button className="admin-mobile-nav-link" type="button" onClick={onMore}><Menu size={18} /><span>More</span></button></nav>
}
...
function AppointmentsView({ data, onAction }: { data: Record<string, any>; onAction: (id: string, action: string) => void }) {
  ...
  const appointments = ((data.appointments || []) as AppointmentRecord[]).filter((appointment) => {
    ...
    return (!query || haystack.includes(query.toLowerCase())) && (status === 'ALL' || appointment.status === status) && (branchId === 'ALL' || appointment.branch?.id === branchId)
  })
  return <div><WorkspaceHeader eyebrow="APPOINTMENTS" title="Appointment inbox." description="Review requests, protect the schedule, and move every visit through a clear status." action={<Link className="admin-button admin-button-primary" href="/calendar"><CalendarDays size={16} /> Open calendar</Link>} /><div className="filter-bar"><label className="search-field"><Search size={17} /><input placeholder="Search name, reference, or phone" value={query} onChange={(event) => setQuery(event.target.value)} /></label><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="ALL">All statuses</option>{['PENDING_REVIEW', 'CONFIRMED', 'RESCHEDULE_PROPOSED', 'CHECKED_IN', 'IN_TREATMENT', 'COMPLETED', 'CANCELLED', 'DECLINED', 'NO_SHOW'].map((item) => <option key={item} value={item}>{statusLabel(item)}</option>)}</select><select value={branchId} onChange={(event) => setBranchId(event.target.value)}><option value="ALL">All branches</option>{BRANCHES.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></div><section className="admin-card table-card"><div className="table-meta"><span><strong>{appointments.length}</strong> records</span><span>All times in {BUSINESS.timezone}</span></div><AppointmentTable appointments={appointments} onAction={onAction} /></section></div>
}

function AppointmentTable({ appointments, onAction }: { appointments: AppointmentRecord[]; onAction?: (id: string, action: string) => void }) {
  return <div className="table-scroll"><table><thead><tr><th>Request</th><th>Patient</th><th>Branch / service</th><th>Preferred time</th><th>Status</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{appointments.map((appointment) => <tr key={appointment.id}><td data-label="Request"><Link className="table-primary" href={'/appointments/' + appointment.id}>{appointment.publicCode}</Link><small>{formatDate(appointment.createdAt)}</small></td><td data-label="Patient"><strong>{appointment.patient?.fullName || '—'}</strong><small>{appointment.patient?.phone || '—'}</small></td><td data-label="Branch / service"><strong>{appointment.branch?.name || '—'}</strong><small>{appointment.service?.name || 'Consultation'}</small></td><td data-label="Preferred time"><strong>{formatDate(appointment.confirmedStartAt || appointment.requestedStartAt)}</strong><small>{appointment.durationMinutes} minutes</small></td><td data-label="Status"><StatusBadge status={appointment.status} /></td><td data-label="Actions"><div className="row-actions">{onAction && <ActionButtons appointment={appointment} onAction={onAction} />}<Link href={'/appointments/' + appointment.id} className="icon-link" aria-label={'Open ' + appointment.publicCode}><ChevronRight size={17} /></Link></div></td></tr>)}</tbody></table>{!appointments.length && <EmptyAdminState title="No appointments match these filters" text="Try a different status or search term." />}</div>
}
